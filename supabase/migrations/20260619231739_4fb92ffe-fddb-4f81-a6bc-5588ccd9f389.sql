...
CREATE OR REPLACE FUNCTION public.get_facebook_config_meta()
 RETURNS TABLE(id uuid, verify_token text, page_id text, app_id text, connected_page_name text,
   token_expires_at timestamp with time zone, ad_account_id text, default_tenant_id uuid,
   last_campaigns_sync_at timestamp with time zone, last_leads_sync_at timestamp with time zone,
   has_page_access_token boolean, has_app_secret boolean,
   last_validated_at timestamp with time zone, last_validation_result jsonb,
   updated_at timestamp with time zone,
   has_user_access_token boolean, user_token_expires_at timestamp with time zone)
...
REVOKE EXECUTE ON FUNCTION public.get_facebook_config_meta() FROM PUBLIC;
