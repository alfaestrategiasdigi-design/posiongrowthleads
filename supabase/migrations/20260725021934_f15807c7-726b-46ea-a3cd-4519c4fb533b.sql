...
BEGIN
  ...
  LOOP
    ...
    -- Unique-indexed FKs: only transfer if the keeper doesn't already have one.
    ...
    -- Remaining refs to losers will be set to NULL on delete (ON DELETE SET NULL).

    -- Loose references (no FK, safe UPDATE).
